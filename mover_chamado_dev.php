...
if (isset($_GET['id'])) {
    ...
    if ($dados = mysqli_fetch_assoc($resultado)) {
        ...
        $sql = "INSERT INTO chamados_dev (id_chamado, titulo_chamado, descricao, whats, data_abertura, hora_abertura, usuario_abertura, categoria_chamado, setor)
                VALUES ('{$dados['id_chamado']}', '{$dados['titulo_chamado']}', '{$dados['descricao']}', '{$dados['whats']}', '{$dados['data_abertura']}', '{$dados['hora_abertura']}', '{$dados['usuario_abertura']}', '{$dados['categoria_chamado']}','desenvolvimento')";
