...
if (isset($_GET['id'])) {
    ...
    $consulta = "SELECT * FROM chamados_finalizados WHERE id_chamado = $id_chamado";
...
//não está feito
    ...
    if ($dados = mysqli_fetch_assoc($resultado)) {
        ...
        $sql = "INSERT INTO chamados (id_chamado, titulo_chamado, descricao, whats, data_abertura, hora_abertura, usuario_abertura, categoria_chamado)
                VALUES ('{$dados['id_chamado']}', '{$dados['titulo_chamado']}', '{$dados['descricao']}', '{$dados['whats']}', '{$dados['data_abertura']}', '{$dados['hora_abertura']}', '{$dados['usuario_abertura']}', '{$dados['categoria_chamado']}')";
